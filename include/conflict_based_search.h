...
class IncidentEdgeIterator
 ...
 private:
   ...
   edge_descriptor dereference() const {
      ...
      std::cout << "Vx " << m_u << " incidence @ " << p << ": " << v << std::endl;
...
inline vertex_descriptor source(edge_descriptor e, const Graph &) {
   return e.first;
}

inline vertex_descriptor target(edge_descriptor e, const Graph &) {
   return e.second;
}
...
class EdgeIterator
 ...
 public:
   ...
   explicit EdgeIterator(const Graph &g, iterator_end)
       : EdgeIterator::iterator_adaptor_(g.n() == 2 ? ++(vertices(g).first)
                                                    : vertices(g).second),
         m_g(&g) {};
...
edges_size_type num_edges(const Graph &g) { return g.n() == 2 ? 1 : g.n(); }
